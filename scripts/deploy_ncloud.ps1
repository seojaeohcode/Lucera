...
try {
    if (-not $SkipUpload -and (-not $SkipArchive -or -not (Test-Path -LiteralPath $archive))) {
        ...
        Invoke-Checked "tar.exe" @(
            ...
            "deploy",
            "config.py",
            "data/db/lucera_minutes.sqlite3"
        )
    }
    ...
    if ($SkipUpload) {
        ...
        $remoteInstallCommand = "tar -xzf /tmp/lucera-runtime.tgz -C /opt/lucera && bash /opt/lucera/deploy/bootstrap.sh && systemctl restart lucera && systemctl reload nginx && for attempt in `$(seq 1 30); do curl --fail --silent --show-error http://127.0.0.1/health && rm -f /tmp/lucera-runtime.tgz && exit 0; sleep 2; done; exit 1"
    }
